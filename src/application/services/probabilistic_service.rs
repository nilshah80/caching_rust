...
impl ProbabilisticService {
    ...
    pub async fn cms_init_by_prob(
        ...
        self.repository
            .cms_init_by_prob(key, error, probability)
            .await
    }

    /// Increment item counts in a Count-Min Sketch (CMS.INCRBY)
    pub async fn cms_incr_by(
        &self,
        key: &str,
    ...
    pub async fn cms_query(
        ...
        key: &str,
        items: Vec<String>,
    ) -> Result<CmsQueryResult, CacheError> {
        self.validate_key(key)?;
    ...
    pub async fn topk_reserve(
        ...
        self.repository
            .topk_reserve(key, k, width, depth, decay)
            .await
    }
    ...
    pub async fn topk_add(
        ...
        key: &str,
        items: Vec<String>,
    ) -> Result<TopKAddResult, CacheError> {
        self.validate_key(key)?;
        self.validate_items(&items)?;
    ...
    pub async fn topk_list(
        &self,
        key: &str,
        with_count: bool,
    ) -> Result<TopKListResult, CacheError> {
        self.validate_key(key)?;
    ...
    fn validate_cms_probability(&self, error: f64, probability: f64) -> Result<(), CacheError> {
        ...
        if probability <= 0.0 || probability >= 1.0 {
            return Err(CacheError::InvalidInput(
                "Probability must be between 0 and 1 (exclusive)".to_string(),
            ));
        }
        Ok(())
    }

    fn validate_topk_params(
        &self,
        k: u64,
        width: Option<u64>,
        depth: Option<u64>,
        ...
        if width.is_some() || depth.is_some() || decay.is_some() {
            // If any optional param is provided, all must be provided
            let (Some(w), Some(d), Some(decay_val)) = (width, depth, decay) else {
                return Err(CacheError::InvalidInput(
            ...
            if decay_val <= 0.0 || decay_val > 1.0 {
                return Err(CacheError::InvalidInput(
                    "Decay must be between 0 (exclusive) and 1 (inclusive)".to_string(),
                ));
            }
        }
        Ok(())
...
mod tests {
    ...
    impl ProbabilisticRepository for MockProbabilisticRepository {
        ...
        async fn cms_incr_by(
            ...
            Ok(CmsIncrByResult {
                key: key.to_string(),
                counts: items.iter().map(|(_, c)| *c).collect(),
            })
        }
        async fn cms_query(
            &self,
    ...
    async fn test_cms_init_by_prob() {
        let service = create_test_service();
        let result = service
            .cms_init_by_prob("cms:test", 0.01, 0.001)
            .await
            .unwrap();
        assert!(result.success);
    }
    ...
    async fn test_topk_reserve() {
        let service = create_test_service();
        let result = service
            .topk_reserve("topk:test", 10, None, None, None)
            .await
            .unwrap();
        assert!(result.success);
    }
    ...
    async fn test_topk_reserve_with_all_params() {
        ...
        let result = service
            .topk_reserve("topk:test", 10, Some(2000), Some(7), Some(0.9))
            .await
            .unwrap();
        assert!(result.success);
    }
    ...
    async fn test_topk_reserve_partial_params_width_only() {
        ...
        let result = service
            .topk_reserve("topk:test", 10, Some(2000), None, None)
            .await;
        assert!(
            matches!(result, Err(CacheError::InvalidInput(msg)) if msg.contains("together or not at all"))
        );
    }
    ...
    async fn test_topk_reserve_partial_params_width_depth() {
        ...
        let result = service
            .topk_reserve("topk:test", 10, Some(2000), Some(7), None)
            .await;
        assert!(
            matches!(result, Err(CacheError::InvalidInput(msg)) if msg.contains("together or not at all"))
        );
    ...
    async fn test_topk_reserve_partial_params_decay_only() {
        ...
        let result = service
            .topk_reserve("topk:test", 10, None, None, Some(0.9))
            .await;
        assert!(
            matches!(result, Err(CacheError::InvalidInput(msg)) if msg.contains("together or not at all"))
        );
    ...
    async fn test_topk_reserve_invalid_width() {
        let service = create_test_service();
        let result = service
            .topk_reserve("topk:test", 10, Some(0), Some(7), Some(0.9))
            .await;
    ...
    async fn test_topk_reserve_invalid_depth() {
        let service = create_test_service();
        let result = service
            .topk_reserve("topk:test", 10, Some(2000), Some(0), Some(0.9))
            .await;
    ...
    async fn test_topk_reserve_invalid_decay() {
        let service = create_test_service();
        let result = service
            .topk_reserve("topk:test", 10, Some(2000), Some(7), Some(1.5))
            .await;
    ...
    async fn test_cms_incr_by_and_query() {
        ...
        let incr_result = service
            .cms_incr_by("cms:test", items.clone())
            .await
            .unwrap();
    ...
    async fn test_pf_add() {
        let service = create_test_service();
        let result = service
            .pf_add("hll:test", vec!["a".to_string(), "b".to_string()])
            .await
            .unwrap();
    ...
    async fn test_pf_count() {
        let service = create_test_service();
        let result = service
            .pf_count(vec!["hll:test".to_string()])
            .await
            .unwrap();
